...
查询是否有可用的寄存器, 将寄存器分配给符号, 查看符号是否有对应寄存器, 置换出寄存器用于分配*/
public class RegisterMap {
    ...
    // 当前未使用(可自由分配)的寄存器
    private HashSet<Integer> freeRegisters = new HashSet<>(allocatableRegisters);

    // 已经分配出去的寄存器对应的符号
    private HashMap<Integer, Symbol> allocatedRegisters = new HashMap<>();

    // 符号对应到寄存器
    private HashMap<Symbol, Integer> symbolToRegister = new HashMap<>();

    // 模拟 LRU 实现 Cache 置换
    private LinkedHashSet<Integer> registerCache = new LinkedHashSet<>();
